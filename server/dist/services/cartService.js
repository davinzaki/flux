...
export const findCartService = async (userId) => {
...
export const addToCartService = async (body, userId) => {
    ...
    const exist = cart.items.find((item) => item.productId.toString() === productId);
    if (exist) {
        exist.qty += qty;
    }
    else {
...
export const updateCartItemService = async (body, userId) => {
    const productId = body.productId;
    const qty = Number(body.qty);
    const product = await findProductByIdService(productId);
    if (product.stock === 0) {
